...
LocalSaveActivity::LocalSaveActivity(std::unique_ptr<SaveFile> newSave, OnSaved onSaved_) :
	...
	onSaved(onSaved_)
{
	ui::Label * titleLabel = new ui::Label(ui::Point(4, 5), ui::Point(Size.X-8, 16), "Сохранить на компьютер:");
	titleLabel->SetTextColour(style::Colour::InformationTitle);
	titleLabel->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	titleLabel->Appearance.VerticalAlign = ui::Appearance::AlignMiddle;
	AddComponent(titleLabel);

	filenameField = new ui::Textbox(ui::Point(8, 25), ui::Point(Size.X-16, 16), save->GetDisplayName(), "[имя файла]");
	...
	ui::Button * cancelButton = new ui::Button(ui::Point(0, Size.Y-16), ui::Point(Size.X-75, 16), "Отмена");
	...
	ui::Button * okayButton = new ui::Button(ui::Point(Size.X-76, Size.Y-16), ui::Point(76, 16), "Сохранить");
...
void LocalSaveActivity::Save()
{
	...
	{
		new ErrorMessage("Ошибка", "Недопустимое имя файла.");
	}
	else if (filenameField->GetText().length())
	{
		...
		{
			new ConfirmPrompt("Перезаписать файл", "Вы уверены, что хотите перезаписать\n"+finalFilename.FromUtf8(), { [this, finalFilename] {
	...
	{
		new ErrorMessage("Ошибка", "Укажите имя файла.");
	}
}
...
void LocalSaveActivity::saveWrite(ByteString finalFilename)
{
	...
	if (saveData.size() == 0)
		new ErrorMessage("Ошибка", "Не удалось сериализовать данные.");
