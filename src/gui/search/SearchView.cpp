	...
	publishButtonShown(false)
{
	...
	nextButton = new ui::Button(ui::Point(WINDOWW-52, WINDOWH-18), ui::Point(50, 16), String("Далее ") + 0xE015);
	previousButton = new ui::Button(ui::Point(2, WINDOWH-18), ui::Point(50, 16), 0xE016 + String(" Назад"));
	tagsLabel  = new ui::Label(ui::Point(270, WINDOWH-18), ui::Point(WINDOWW-540, 16), "\boПопулярные теги:");
	...
	pageLabel = new ui::Label(ui::Point(0, WINDOWH-18), ui::Point(30, 16), "Страница"); //page [TEXTBOX] of y
	...
	searchField = new ui::Textbox(ui::Point(60, 10), ui::Point(WINDOWW-283, 17), "", "[поиск, F1 — помощь]");
	...
	dateRange->AddOption({"Все", 0});
	dateRange->AddOption({"День", 1});
	dateRange->AddOption({"Неделя", 2});
	dateRange->AddOption({"Месяц", 3});
	dateRange->AddOption({"Год", 4});
	...
	sortButton = new ui::Button(ui::Point(WINDOWW-140, 10), ui::Point(61, 17), "Сортировка");
	...
	ownButton = new ui::Button(ui::Point(WINDOWW-70, 10), ui::Point(61, 17), "Мои");
	...
	ui::Label * searchPrompt = new ui::Label(ui::Point(10, 10), ui::Point(50, 16), "Поиск:");
	...
	removeSelected = new ui::Button(ui::Point(((WINDOWW-415)/2), WINDOWH-18), ui::Point(100, 16), "Удалить");
	removeSelected->Visible = false;
	removeSelected->SetActionCallback({ [this] { c->RemoveSelected(); } });
	AddComponent(removeSelected);

	unpublishSelected = new ui::Button(ui::Point(((WINDOWW-415)/2)+105, WINDOWH-18), ui::Point(100, 16), "Снять");
	...
	favouriteSelected = new ui::Button(ui::Point(((WINDOWW-415)/2)+210, WINDOWH-18), ui::Point(100, 16), "В избранное");
	...
	clearSelection = new ui::Button(ui::Point(((WINDOWW-415)/2)+315, WINDOWH-18), ui::Point(100, 16), "Снять выбор");
...
{
	String info =
		"Введите запрос в поле поиска — поиск по названиям и тегам начнётся автоматически. Условия объединяются через OR.\n"
		"\n"
		"Сортировка: нажмите кнопки \bt«По голосам»\bw / \bt«По дате»\bw для изменения порядка отображения сохранений\n"
		"Категории: если вы вошли в аккаунт, используйте \bt«Мои»\bw для просмотра только своих сохранений, или нажмите на звезду для избранных\n"
		"Период: выберите в выпадающем списке справа от поля поиска диапазон дат\n"
		"\n"
		"Специальные условия поиска:\n"
		"\btid:#######\bw - поиск по id сохранения\n"
		"\bthistory:#######\bw - предыдущие версии сохранения по id\n"
		"\btuser:XXXXXX\bw - поиск сохранений конкретного пользователя\n"
		"\btbefore:YYYY-MM-DD\bw - все сохранения созданные до указанной даты. Месяц и день необязательны\n"
		"\btafter:YYYY-MM-DD\bw - все сохранения созданные после указанной даты. Месяц и день необязательны\n"
		"\n"
		"Расширенный поиск:\n"
		"Начните запрос с \bt~\bw для расширенного поиска. Поиск работает по названиям, описаниям, именам пользователей и тегам, а не только по названиям и тегам."
		" Условия объединяются через AND вместо OR.\n"
		"Используйте \bt|\bw для OR между условиями, например \bg~bomb | nuke | explosive\bw\n"
		"Используйте \bt!\bw для исключения условий, например \bg~city !destroyable !desert\bw\n"
		"Используйте \bt\"\bw для многословных условий, например \bg~\"power plant\" uran | plut | polo\bw\n"
		"Используйте \bt@title\bw для поиска только в названиях, например \bg~@title subframe\bw\n"
		"Используйте \bt@description\bw для поиска только в описаниях, например \bg~@description \"No description provided\"\bw\n"
		"Используйте \bt@user\bw для поиска по пользователям, например \bg~@user 117n00b | Catelite | Fluttershy @title laser\bw\n"
		"Используйте \bt@tags\bw для поиска только по тегам, например \bg~@tags resistcup @title printer | @description spider before:2024-06\bw\n"
		"Скобки позволяют создавать более сложные запросы. Например: \bg~(@user MG99 @description complete) | (@user goglesq @tags tutorial)\bw"
		;

	new InformationMessage("Помощь по поиску", info, true);
}
...
void SearchView::NotifySortChanged(SearchModel * sender)
{
	if(sender->GetSort() == http::sortByVotes)
	{
		...
		sortButton->SetText("По голосам");
		sortButton->SetIcon(IconVoteSort);
	}
	else
	{
		sortButton->SetToggleState(true);
		sortButton->SetText("По дате");
...
void SearchView::NotifyPageChanged(SearchModel * sender)
{
	...
	{
		String pageInfo = String::Build("из ", pageCount);
...
void SearchView::NotifySaveListChanged(SearchModel * sender)
{
	...
	if(sender->GetShowFavourite())
		favouriteSelected->SetText("Убрать из избранного");
	else
		favouriteSelected->SetText("В избранное");
	...
	if (!saves.size())
	{
		...
		if (!errorLabel)
		{
			errorLabel = new ui::Label(ui::Point(0, (WINDOWH/2)-6), ui::Point(WINDOWW, 12), "Ошибка");
			AddComponent(errorLabel);
		}
		if (!sender->GetSavesLoaded())
		{
			errorLabel->SetText("Загрузка...");
		...
		else
		{
			if(sender->GetLastError().length())
				...
				errorLabel->SetText("\boСохранения не найдены");
		}
	}
	else
	{
...
void SearchView::NotifySelectedChanged(SearchModel * sender)
{
	...
	if (selected.size())
	{
		...
		{
			unpublishSelected->SetText("Опубликовать");
			publishButtonShown = true;
		}
		else
		{
			unpublishSelected->SetText("Снять");
			publishButtonShown = false;
		}
	}
