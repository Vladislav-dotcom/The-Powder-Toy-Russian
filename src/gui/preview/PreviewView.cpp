	...
	commentHelpText(false)
{
	...
	favButton = new ui::Button(ui::Point(50, Size.Y-19), ui::Point(51, 19), "Избр.");
	...
	reportButton = new ui::Button(ui::Point(100, Size.Y-19), ui::Point(51, 19), "Жалоба");
	...
	reportButton->SetActionCallback({ [this] {
		new TextPrompt("Пожаловаться на сохранение", "При отправке жалобы учитывайте следующее:\n\bw1)\bg При жалобе на украденные сохранения укажите ID оригинального сохранения.\n\bw2)\bg Не просите убирать сохранения с главной страницы, если они не нарушают правила.\n\bw3)\bg Жалобу можно отправить и на комментарии или теги (в том числе к вашим сохранениям).", "", "[reason]", true, { [this](String const &resultText) {
	...
	openButton = new ui::Button(ui::Point(0, Size.Y-19), ui::Point(51, 19), "Открыть");
	...
	browserOpenButton = new ui::Button(ui::Point((XRES/2)-107, Size.Y-19), ui::Point(108, 19), "Открыть в браузере");
	...
	loadErrorButton = new ui::Button({ 0, 0 }, ui::Point(148, 19), "Ошибка загрузки сохранения");
	...
	missingElementsButton = new ui::Button({ 0, 0 }, ui::Point(148, 19), "Нет пользовательских элементов");
	...
	fromNewerVersionButton = new ui::Button({ 0, 0 }, ui::Point(148, 19), "Сохранение из новой версии");
	...
	pageInfo = new ui::Label(ui::Point((XRES/2) + 85, Size.Y+1), ui::Point(70, 16), "Страница 1 из 1");
...
void PreviewView::AttachController(PreviewController * controller)
{
	...
	int textWidth = Graphics::TextSize("Нажмите поле ниже, чтобы скопировать ID сохранения").X - 1;
	saveIDLabel = new ui::Label(ui::Point((Size.X-textWidth-20)/2, Size.Y+5), ui::Point(textWidth+20, 16), "Нажмите поле ниже, чтобы скопировать ID сохранения");
	...
	saveIDLabel2 = new ui::Label(ui::Point((Size.X-textWidth-20)/2-37, Size.Y+22), ui::Point(40, 16), "ID сохранения:");
...
void PreviewView::CheckComment()
{
	...
	{
		commentWarningLabel->SetText("Отправка комментария...");
	...
	else if (!userIsAuthor && (text.Contains("stolen") || text.Contains("copied")))
	{
		...
		{
			if (interfaceRng()%2)
				commentWarningLabel->SetText("Украдено? Лучше пожаловаться на сохранение");
			else
				commentWarningLabel->SetText("Сообщайте об украденных сохранениях через жалобу");
			commentHelpText = true;
		}
	}
	else if (userIsAuthor && text.Contains("vote"))
	{
		commentWarningLabel->SetText("Не просите голоса");
		commentHelpText = true;
	}
	else if (CheckSwearing(text))
	{
		...
		{
			if (interfaceRng()%2)
				commentWarningLabel->SetText("Не используйте ненормативную лексику");
			else
				commentWarningLabel->SetText("Комментарии с нецензурной лексикой могут быть удалены");
			commentHelpText = true;
		}
	}
	else
...
void PreviewView::DoDraw()
{
	...
	if (c->GetDoOpen() && !doError)
	{
		...
		g->BlendText(Position + Vec2{(Size.X/2)-((Graphics::TextSize("Загрузка сохранения...").X - 1)/2), (Size.Y/2)-5}, "Загрузка сохранения...", style::Colour::InformationTitle.NoAlpha().WithAlpha(255));
	}
	if (!c->GetFromUrl() || doError)
	{
...
void PreviewView::OnTick()
{
	...
	if (reportSaveRequest && reportSaveRequest->CheckDone())
	{
		try
		{
			...
			c->Exit();
			new InformationMessage("Информация", "Жалоба отправлена", false);
		}
		catch (const http::RequestError &ex)
		{
			new ErrorMessage("Ошибка", "Не удалось отправить жалобу: " + ByteString(ex.what()).FromUtf8());
		}
	...
	if (addCommentRequest && addCommentRequest->CheckDone())
	{
		...
		{
			new ErrorMessage("Ошибка отправки комментария", ByteString(ex.what()).FromUtf8());
		}
...
{
	new ErrorMessage("Ошибка загрузки сохранения", doErrorMessage, {});
}

void PreviewView::ShowMissingCustomElements()
{
	StringBuilder sb;
	sb << "В этом сохранении используются пользовательские элементы, которые сейчас недоступны. Убедитесь, что установлен нужный мод и/или все скрипты, необходимые для полной загрузки сохранения.";
	auto remainingIds = missingElements.ids;
	if (missingElements.identifiers.size())
	{
		sb << "\n\nНиже приведён список идентификаторов отсутствующих пользовательских элементов — это может помочь решить проблему.\n";
	...
	{
		sb << "\n\nНиже приведён список ID отсутствующих пользовательских элементов без связанного идентификатора. Исправить это может только автор сохранения.\n";
		for (auto id : remainingIds)
		{
			sb << "\n - " << id;
		}
	}
	new InformationMessage("Нет пользовательских элементов", sb.Build(), true);
}
...
void PreviewView::ShowFromNewerVersion()
{
	...
	{
		new InformationMessage("Это сохранение из снимка или бета-версии", String::Build("Установите снимок или бета-версию: ", SERVER), false);
	}
	else if (fromNewerVersion)
	{
		new InformationMessage("Это сохранение из более новой версии", String::Build("Обновите TPT в игре или на ", SERVER), false);
	}
}
...
void PreviewView::NotifySaveChanged(PreviewModel * sender)
{
	...
	if(save)
	{
		...
		{
			authorDateLabel->SetText("\bgАвтор:\bw " + save->userName.FromUtf8() + " \bg" + dateType + " \bw" + format::UnixtimeToDateMini(save->updatedDate).FromAscii());
		}
		auto user = Client::Ref().GetAuthUser();
		if (user && save->userName == user->Username)
			...
			userIsAuthor = false;
		viewsLabel->SetText(String::Build("\bgПросмотры:\bw ", save->Views));
...
void PreviewView::submitComment()
{
	if (addCommentBox)
	{
		...
		{
			new ErrorMessage("Ошибка", "Комментарий слишком короткий");
		}
		else
		{
...
void PreviewView::NotifyCommentBoxEnabledChanged(PreviewModel * sender)
{
	...
	{
		addCommentBox = new ui::Textbox(ui::Point((XRES/2)+4, Size.Y-19), ui::Point(Size.X-(XRES/2)-48, 17), "", "Добавить комментарий");
		...
		submitCommentButton = new ui::Button(ui::Point(Size.X-40, Size.Y-19), ui::Point(40, 19), "Отправить");
		submitCommentButton->SetActionCallback({ [this] { submitComment(); } });
		AddComponent(submitCommentButton);

		commentWarningLabel = new ui::Label(ui::Point((XRES/2)+4, Size.Y-19), ui::Point(Size.X-(XRES/2)-48, 16), "Если вы это видите — это ошибка");
	...
	{
		submitCommentButton = new ui::Button(ui::Point(XRES/2, Size.Y-19), ui::Point(Size.X-(XRES/2), 19), "Войдите, чтобы комментировать");
...
{
	pageInfo->SetText(String::Build("Страница ", sender->GetCommentsPageNum(), " из ", sender->GetCommentsPageCount()));
}
