	...
	doErrorMessage("")
{
	...
	ui::Button * closeButton = new ui::Button(ui::Point(0, Size.Y-15), ui::Point(Size.X, 15), "Закрыть");
	...
	if(editable)
	{
		...
		ui::Button * saveButton = new ui::Button(ui::Point(Size.X/2, Size.Y-15), ui::Point(Size.X/2, 15), "Сохранить");
...
void ProfileActivity::setUserInfo(UserInfo newInfo)
{
	...
	if (!info.biography.length() && !editable)
		info.biography = "\bgНе указано";
	if (!info.location.length() && !editable)
		info.location = "\bgНе указано";
	if (!info.website.length())
		info.website = "\bgНе указано";
	...
	if (editable)
	{
		ui::Button * editAvatar = new ui::Button(ui::Point(Size.X - (40 + 16 + 75), currentY), ui::Point(75, 15), "Изменить аватар");
	...
	// age
	ui::Label * ageTitle = new ui::Label(ui::Point(4, currentY), ui::Point(23, 15), "Возраст:");
	...
	ui::Label *age = new ui::Label(ui::Point(5+ageTitle->Size.X, currentY), ui::Point(Size.X-ageTitle->Size.X-56, 15), info.age ? String::Build(info.age) : "\bgНе указано");
	age->Appearance.HorizontalAlign = ui::Appearance::AlignLeft;
	scrollPanel->AddChild(age);
	currentY += 2+age->Size.Y;

	// location
	ui::Label * locationTitle = new ui::Label(ui::Point(4, currentY), ui::Point(45, 15), "Местоположение:");
	...
	// website
	ui::Label * websiteTitle = new ui::Label(ui::Point(1, currentY), ui::Point(42, 15), "Сайт:");
	...
	// saves
	ui::Label * savesTitle = new ui::Label(ui::Point(4, currentY), ui::Point(35, 15), "Сохранения:");
	...
	currentY += savesTitle->Size.Y;
		...
		// saves count
		ui::Label * saveCountTitle = new ui::Label(ui::Point(12, currentY), ui::Point(32, 15), "Количество:");
		...
		// average score
		ui::Label * averageScoreTitle = new ui::Label(ui::Point(12, currentY), ui::Point(72, 15), "Средний балл:");
		...
		// highest score
		ui::Label * highestScoreTitle = new ui::Label(ui::Point(12, currentY), ui::Point(71, 15), "Лучший балл:");
	...
	// biograhy
	ui::Label * bioTitle = new ui::Label(ui::Point(4, currentY), ui::Point(50, 15), "О себе:");
...
void ProfileActivity::OnTick()
{
	if (doError)
	{
		new ErrorMessage("Ошибка", doErrorMessage, { [this]() {
			Exit();
		} });
	}
	...
	if (saveUserInfoRequest && saveUserInfoRequest->CheckDone())
	{
		...
		{
			doError = true;
			doErrorMessage = "Не удалось сохранить профиль: " + ByteString(ex.what()).FromUtf8();
		}
	...
	if (getUserInfoRequest && getUserInfoRequest->CheckDone())
	{
		...
		{
			doError = true;
			doErrorMessage = "Не удалось загрузить профиль: " + ByteString(ex.what()).FromUtf8();
		}
